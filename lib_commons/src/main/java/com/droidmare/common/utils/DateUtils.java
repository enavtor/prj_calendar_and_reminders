...
import com.droidmare.common.models.DateDataStructures;
...
public class DateUtils {
    ...
    public static boolean notSameDate(int eventDay, int eventMonth, int eventYear) {
        return (currentDay != eventDay || currentMonth != eventMonth || currentYear != eventYear);
    }

    //This method returns true if the event's date is the same as the DateUtils current date:
    public static boolean sameDate(int eventDay, int eventMonth, int eventYear) {
        return (currentDay == eventDay && currentMonth == eventMonth && currentYear == eventYear);
    }
    ...
    //Wrapper for the calculateNextRepetition original method:
    public static long calculateNextRepetition (EventJsonObject eventJson, boolean willBeDisplayed, long repetitionStop, int intervalTime) {
        ...
        if (willBeDisplayed) {
            ...
            if (minute == 59) {
                ...
                int[] incrementedDateArray = DateUtils.getIncrementedDate(calendar.getTimeInMillis(), 1);
                calendar.setTimeInMillis(DateUtils.getMillisFromArray(incrementedDateArray));
            }
        ...
        long nextRepetition = calculateNextRepetition(
            eventJson.getString(ConstantValues.EVENT_REPETITION_TYPE_FIELD, ""),
            currentDate,
            eventJson.getLong(ConstantValues.EVENT_START_DATE_FIELD, -1),
            repetitionStop,
            intervalTime
        );
    ...
    public static long calculateNextRepetition (String repetitionType, long currentDate, long eventDateMillis, long repetitionStop, int intervalTime) {
        ...
        if (intervalTime != 0) {
            ...
            try {
                ...
                repConfig = EventJsonObject.getRepetitionConfigArray(repTypeAux.getString("config"));
    ...
    private static long getNextDailyRepetition(long currentDate, long eventStartDate, long eventStopDate, int intervalTime) {
    ...
    private static long getNextAlternateRepetition(long currentDate, long eventStartDate, long eventStopDate, ArrayList<Integer> repetitionConfig, int intervalTime) {
    ...
    private static long getNextWeeklyRepetition(long currentDate, long eventStartDate, long eventStopDate, ArrayList<Integer> repetitionConfig, int intervalTime) {
        ...
        int currentDayOfWeek = getDayOfWeek(currentDateArray[DAY], currentDateArray[MONTH], currentDateArray[YEAR]);
    ...
    private static long getNextMonthlyRepetition(long currentDate, long eventStartDate, long eventStopDate, ArrayList<Integer> repetitionConfig, int intervalTime) {
        ...
        int currentDayOfMonth = currentDateArray[DAY];
    ...
    private static long getNextAnnualRepetition(long currentDate, long eventStartDate, long eventStopDate, int intervalTime) {
    ...
    private static int[] getIncrementedDate (long date, int numberOfHours) {
    ...
    public static int findDayOfWeek() {
            double a = Math.floor((14 - ((double)currentMonth + 1)) / 12);
    ...
    public static int findDayOfWeek(int month, int year) {
        double a = Math.floor((14 - ((double)month + 1)) / 12);
        double y = year - a;
    ...
    //This method returns a string with the date, passed as an array, formatted:
    public static String getFormattedDate(int[] dateArray) {

        String dateText = DateUtils.formatDateText(dateArray[DAY], dateArray[MONTH], dateArray[YEAR]) + " " ;
        String timeText = DateUtils.formatTimeString(dateArray[HOUR]) + ":" + DateUtils.formatTimeString(dateArray[MINUTE]);

        return dateText + timeText;
    }

    //Method that takes a fully formatted date and returns an array formed by its elements (day, month, etc):
    public static int[] getDateValues(String fullDate) {
    ...
    public static String getTodayFormattedDate() {
        return getFormattedDate(transformFromMillis(getTodayMillis()));
    }
